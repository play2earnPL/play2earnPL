...
final class AmpResponse implements ResponseInterface, StreamableInterface
{
    ...
    private static function generateResponse(Request $request, AmpClientState $multi, string $id, array &$info, array &$headers, CancellationTokenSource $canceller, array &$options, Closure $onProgress, &$handle, $logger, Promise &$pause) : Generator
    {
    ...
    private static function followRedirects(Request $originRequest, AmpClientState $multi, array &$info, array &$headers, CancellationTokenSource $canceller, array $options, Closure $onProgress, &$handle, $logger, Promise &$pause) : Generator
    {
        (yield $pause);
    ...
    private static function getPushedResponse(Request $request, AmpClientState $multi, array &$info, array &$headers, array $options, $logger) : Generator
    {
        if ('' !== $options['body']) {
            return null;
        }
